...
.mode-badge.dialogue { background: #1a2040; color: #4285f4; border: 1px solid #2d3560; }
.mode-badge.script   { background: #1a2a20; color: #34a853; border: 1px solid #2d5040; }

.var-card { border-radius: 12px; padding: 20px 24px; margin-bottom: 12px; border-left: 4px solid; }
.var-1 { background: #12172b; border-color: #4285f4; }
.var-2 { background: #12231a; border-color: #34a853; }
.var-3 { background: #231820; border-color: #ea4335; }
.var-4 { background: #1a1a12; border-color: #fbbc04; }
.var-5 { background: #12202a; border-color: #06b6d4; }
.var-title { font-family: 'Syne', sans-serif; font-weight: 700; font-size: 0.95rem; margin-bottom: 12px; letter-spacing: 0.03em; }
.var-1 .var-title { color: #4285f4; }
.var-2 .var-title { color: #34a853; }
.var-3 .var-title { color: #ea4335; }
.var-4 .var-title { color: #fbbc04; }
.var-5 .var-title { color: #06b6d4; }
.var-content { color: #c8cad8; line-height: 1.7; white-space: pre-wrap; font-size: 0.9rem; }

.chat-msg-user {
    background: #1a1d2e; border: 1px solid #252840;
    border-radius: 12px 12px 4px 12px;
    padding: 12px 16px; margin: 8px 0 8px 40px;
    color: #e8eaf0; font-size: 0.9rem;
}
.chat-msg-bot {
    background: #111318; border: 1px solid #1e2130;
    border-radius: 12px 12px 12px 4px;
    padding: 12px 16px; margin: 8px 40px 8px 0;
    color: #c8cad8; font-size: 0.9rem;
}
.kb-chip {
    display: inline-flex; align-items: center; gap: 6px;
    background: #1a1d2e; border: 1px solid #252840;
    border-radius: 8px; padding: 6px 10px;
    font-size: 0.78rem; color: #9a9db0; margin: 3px 0; width: 100%;
}
.hist-item {
...
.info-box {
    background: #12172b; border: 1px solid #252840;
    border-radius: 10px; padding: 12px 16px;
    font-size: 0.85rem; color: #9a9db0;
}
.section-label {
    font-family: 'Syne', sans-serif; font-size: 0.78rem; font-weight: 700;
    color: #5a5d70; letter-spacing: 0.1em; text-transform: uppercase;
    margin-bottom: 10px; padding-bottom: 6px; border-bottom: 1px solid #1e2130;
}
.gen-header {
    font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700;
    color: #e8eaf0; margin: 24px 0 16px;
    display: flex; align-items: center; gap: 10px;
}
.stButton > button { border-radius: 10px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 500 !important; transition: all 0.2s !important; }
.stButton > button[kind="primary"] { background: linear-gradient(135deg, #4285f4, #34a853) !important; border: none !important; color: white !important; }
.stButton > button[kind="primary"]:hover { transform: translateY(-1px) !important; box-shadow: 0 4px 16px rgba(66,133,244,0.35) !important; }
.stTextArea textarea, .stTextInput input { background: #111318 !important; border: 1px solid #1e2130 !important; border-radius: 10px !important; color: #e8eaf0 !important; }
.stTextArea textarea:focus, .stTextInput input:focus { border-color: #4285f4 !important; box-shadow: 0 0 0 2px rgba(66,133,244,0.15) !important; }
.stTabs [data-baseweb="tab-list"] { background: #111318; border-radius: 10px; padding: 4px; gap: 4px; }
.stTabs [data-baseweb="tab"] { border-radius: 8px !important; color: #7a7d8e !important; font-family: 'Syne', sans-serif !important; font-weight: 600 !important; font-size: 0.82rem !important; }
.stTabs [aria-selected="true"] { background: #1a1d2e !important; color: #4285f4 !important; }
hr { border-color: #1e2130 !important; margin: 20px 0 !important; }
.stTextArea label, .stTextInput label, .stSelectbox label { color: #7a7d8e !important; font-size: 0.82rem !important; font-weight: 500 !important; letter-spacing: 0.04em !important; text-transform: uppercase !important; }
...
def build_system(mode, kb):
    ...
    if mode == "dialogue":
        ...
        if kb.get("dialogues"):
            items = "\n\n---\n\n".join(
                f"[{d['name']}]\n{d['content']}" for d in kb["dialogues"][-6:]
            )
...
WHEN GENERATING DIALOGUES — always produce EXACTLY 5 labeled variations in this exact format:

## VARIATION 1: SHORT & SWEET
Brief, crisp, confident. Gets to the point in 2–4 exchanges. No fluff.

## VARIATION 2: PROFESSIONAL
Formal, structured, polished. Respectful honorifics throughout.

## VARIATION 3: FRIENDLY & CONVERSATIONAL
Warm, casual, relatable Hinglish. Feels like a helpful friend, not a salesperson.

## VARIATION 4: PERSUASIVE
Benefit-led, creates urgency without pressure. Highlights key differentiators.

## VARIATION 5: EMPATHETIC
Deeply understanding of customer concerns. Validates feelings first, then guides gently.
...
{kb_section}"""
        ...
        if kb.get("scripts"):
            items = "\n\n---\n\n".join(
                f"[{s['name']}]\n{s['content']}" for s in kb["scripts"][-6:]
            )
...
WHEN GENERATING SCRIPTS — always produce EXACTLY 5 labeled variations in this exact format:

## VARIATION 1: MINIMAL SCRIPT
Streamlined flow with only essential steps. Fast, lean, low token cost.

## VARIATION 2: COMPREHENSIVE SCRIPT
Full branching logic covering all edge cases: YES/NO/BUSY/VOICEMAIL/WRONG_PERSON/CALLBACK flows.

## VARIATION 3: CUSTOMER-CENTRIC SCRIPT
Prioritises customer journey and emotional experience. Extra empathy checkpoints.

## VARIATION 4: CONVERSION-OPTIMISED SCRIPT
Every branch designed to maximise RM callback scheduling. Smart re-engagement on first rejection.

## VARIATION 5: OBJECTION-RECOVERY SCRIPT
Specialised for "not interested", "already have insurance", "too expensive" scenarios with layered persuasion.
...
- Specify validation rules where applicable
...
with st.sidebar:
    ...
    <div style="padding:12px 0 20px">
        ...
        <div style="font-size:0.75rem;color:#5a5d70;margin-top:2px">ABHI · Powered by Gemini</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for chat in history:
        ...
        with col_a:
            st.markdown(f"""<div class="hist-item" style="{border}">
                <div class="hist-title">{mode_icon} {title}</div>
                <div class="hist-date">{ts}</div>
            </div>""", unsafe_allow_html=True)
        with col_b:
            if st.button("›", key=f"open_{chat['id']}"):
                load_chat(chat["id"])
                st.rerun()
    ...
    with kb_tab1:
        ...
        with st.expander("＋ Add Script"):
            ...
            sc = st.text_area("Paste script", key="sc", height=120)
    ...
    with kb_tab2:
        ...
        with st.expander("＋ Add Dialogue"):
            ...
            dc = st.text_area("Paste dialogue", key="dc", height=120)
...
mode_desc  = "Generating natural conversation dialogues" if mode == "dialogue" else "Generating structured flow scripts with branching logic"

st.markdown(f"""
<div class="app-header">
    <div>
        <h1>DialogueBot</h1>
        <p>ABHI Voice Bot Optimizer — {mode_desc} · Gemini 1.5 Flash</p>
    </div>
    <div style="margin-left:auto">
        <span class="mode-badge {'dialogue' if mode=='dialogue' else 'script'}">{mode_label}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
with c1:
    if st.button("💬 Dialogue Mode", use_container_width=True, type="primary" if mode == "dialogue" else "secondary"):
        st.session_state.mode = "dialogue"
        st.session_state.generated_dialogues = None
        st.rerun()
with c2:
    if st.button("📋 Script Mode", use_container_width=True, type="primary" if mode == "script" else "secondary"):
...
with col_right:
    ...
    with pcol1:
        language = st.selectbox("Language", ["Hinglish (Hindi + English)", "Pure Hindi", "Pure English"], label_visibility="collapsed")
    with pcol2:
        tone = st.selectbox("Tone", ["Balanced", "More Formal", "More Casual", "More Persuasive", "More Empathetic"], label_visibility="collapsed")
...
if generate_clicked:
    if not context.strip() and not instructions.strip():
        st.warning("Please provide at least a context or instructions.")
    else:
        ...
        st.session_state.messages.append({"role": "user",      "content": f"[GENERATE {mode.upper()}] {summary}"})
            ...
            with tab:
                ...
                with b2:
                    if st.button("💾 Save to KB", key=f"kb_{i}", use_container_width=True):
                        kb_fresh = load_kb()
                        entry = {"id": str(uuid.uuid4()), "name": var["title"], "content": var["content"], "added": datetime.now().isoformat()}
                        kb_fresh["dialogues" if mode == "dialogue" else "scripts"].append(entry)
                        save_kb(kb_fresh)
...
st.markdown('<div style="text-align:center;padding:40px 0 20px;font-size:0.75rem;color:#3a3d50">DialogueBot · ABHI Voice Bot Optimizer · Gemini 1.5 Flash</div>', unsafe_allow_html=True)